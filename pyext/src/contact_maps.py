...
class CMTable(object):
    ...
    def write_matrices(self):
        ...
        for key in self.cm_all.keys():
            np.savetxt(
                os.path.join(self.out_dir, 'ContMap_%s.dat' % key),
                np.array(self.cm_all[key]), fmt='%s')
        ...
        for name1,  name2 in itertools.combinations(self.resi_dict, 2):
            ...
            if np.sum(mat) > 0:
                ...
                for i in np.transpose(loc):
                    contacts.append(self.resi_dict[p1][i[1]]
                                    + self.resi_dict[p2][i[0]] + [frq[k]])
                    k += 1
                # Sort array
                contacts = np.array(contacts)
                sort_contacts = contacts[
                    contacts[:, 4].astype(float).argsort()]
                np.savetxt(os.path.join(
                    self.out_dir, 'contacts_%s_%s.dat' % (p1, p2)),
                    sort_contacts[::-1], fmt='%s')
                cont_dict[p1+'-'+p2] = contacts

    def plot_contact_maps(self, filename='contact_map.pdf'):
        ...
        for i, p1 in enumerate(nres.keys()):
            for j, p2 in enumerate(nres.keys()):
                ...
                if p1+'-'+p2 in self.cm_all.keys():
                    if np.shape(self.cm_all[p1+'-'+p2])[0] \
                            == self.n_residues[p1]:
                        M = np.transpose(self.cm_all[p1+'-'+p2])
                    else:
                        M = np.transpose(self.cm_all[p1+'-'+p2])
                else:
                    if np.shape(self.cm_all[p2+'-'+p1])[0] \
                            == self.n_residues[p2]:
                        M = self.cm_all[p2+'-'+p1]
                    else:
                        M = self.cm_all[p2+'-'+p1]
                    ...
                    for xl in sXLs.keys():
                        ...
                        if p1 == p2:
                            if np.min(sXLs[xl]) < self.XLs_cutoff:
                                ...
                                ax.scatter(r2, r1, s=area, color='greenyellow',
                                           alpha=0.7, edgecolors='none')
                            else:
                                ax.scatter(r1, r2, s=area, color='orangered',
                                           alpha=0.7, edgecolors='none')
        ...
        for i in range(nprot):
            fig.get_axes()[i].set_title('%s' % (list(nres.keys())[i]),
                                        fontsize=8)
        # Add vertical labels
        fig.get_axes()[0].set_ylabel('%s' % (list(nres.keys())[0]),
                                     fontsize=8)
        k = 1
        for i in range(nprot, nprot*nprot, nprot):
            fig.get_axes()[i].set_ylabel('%s' % (list(nres.keys())[k]),
                                         fontsize=8)
            k += 1
        fig.tight_layout()
    ...
    def plot_contact_maps_subunits(self):
        ...
        for i, p in enumerate(combinations):
            ...
            ax.tick_params(axis='x', bottom=True, top=False,
                           labelbottom=True, labeltop=False, labelsize=20)
    ...
    def _get_particles_at_lowest_resolution(self, hier):
        ...
        for mol in IMP.atom.get_by_type(hier,
            ...
            if len(self.selection) == 0 or mol.get_name() in self.selection:
                ...
                particles_dict[mol.get_name()+'.'+str(copy)] \
