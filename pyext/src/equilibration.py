...
"""
Utilities for automatically detecting equilibrated region of molecular
simulations.
...
def statisticalInefficiency_multiscale(A_n, B_n=None, fast=False, mintime=3):
    ...
    fast : bool, optional, default=False
        f True, will use faster (but less accurate) method to estimate
        correlation time, described in Ref. [1] (default: False).
    mintime : int, optional, default=3
        ...
        to mintime when the correlation function first goes negative.
        Note that this time may need to be increased if there is a strong
    ...
    Compute statistical inefficiency of timeseries data with known
    correlation time.
    ...
    # Compute estimator of covariance of (A,B) using estimator that
    # will ensure C(0) = 1.
    ...
    # Accumulate the integrated correlation time by computing the
    # normalized correlation time at increasing values of t.  Stop
    # accumulating if the correlation function goes negative, since this is
    # unlikely to occur unless the correlation function has decayed to the
    # point where it is dominated by noise and indistinguishable from zero.
    t = 1
    increment = 1
    while (t < N - 1):
        ...
        # Terminate if the correlation function has crossed zero and
        # we've computed the correlation function at least out to 'mintime'.
...
def statisticalInefficiency_geyer(A_n, method='pos'):
    """Compute the statistical inefficiency of a timeseries using the
       methods of Geyer.
    ...
    method : str, optional, default='con'
        The method to use; matches notation from `initseq` from `mcmc`
        R package by Geyer.
    ...
    Notes
    -----
    Implementation based on the `initseq` method from the `mcmc` R package
    by Geyer.

    References
    ----------
    [1] Geyer, CJ. Practical Markov chain Monte Carlo.
        Statistical Science 7(4):473-511, 1992.

    Examples
    --------

    Compute statistical inefficiency of timeseries data with known
    correlation time using different methods.
    ...
    if method not in ['pos', 'dec', 'con']:
        raise TypeError("Unknown method '%s'; must be one of "
                        "['pos', 'dec', 'con']" % method)
    ...
    for j in range(1, ngamma):
        ...
        nuff[nstep] = 1
        nstep += 1
        while (nstep > 1) and ((puff[nstep-1] / nuff[nstep-1]) <
    ...
    # Compute statistical inefficiencies from sample mean
    # var = var(A_n) / (N/g)
    # g = var / (var(A_n)/N)
...
def detectEquilibration(A_t, nskip=1, method='multiscale'):
    """Automatically detect equilibrated region of a dataset using a
       heuristic that maximizes number of effectively uncorrelated samples.
    ...
    nskip : int, optional, default=1
        number of samples to sparsify data by in order to speed
        equilibration detection
    method : str, optional, default='geyer'
        Method to use for computing statistical inefficiency; one of
        ['geyer', 'multiscale']
    ...
    >>> A_t = testsystems.correlated_timeseries_example(N=1000, tau=10.0)
    ...
    >>> from pymbar import testsystems
    >>> # generate a test correlated timeseries
    >>> A_t = testsystems.correlated_timeseries_example(N=1000, tau=10.0) + 2.0
    >>> # generate a test correlated timeseries
    >>> B_t = testsystems.correlated_timeseries_example(N=10000, tau=10.0)
    >>> C_t = np.concatenate([A_t, B_t])
    >>> # compute indices of uncorrelated timeseries
    >>> [t, g, Neff_max] = detectEquilibration(C_t, nskip=50)
