...
color_palette = [u'#4c72b0', u'#55a868', u'#c44e52', u'#8172b2', u'#ccb974',
                 u'#64b5cd', u'#4c72b0', u'#55a868', u'#c44e52', u'#8172b2',
                 u'#ccb974', u'#64b5cd', u'#4c72b0', u'#55a868', u'#c44e52',
                 u'#8172b2', u'#ccb974', u'#64b5cd', u'#4c72b0', u'#55a868',
                 u'#c44e52', u'#8172b2', u'#ccb974', u'#64b5cd', u'#4c72b0',
                 u'#55a868', u'#c44e52', u'#8172b2', u'#ccb974', u'#64b5cd',
                 u'#4c72b0', u'#55a868', u'#c44e52', u'#8172b2', u'#ccb974',
                 u'#64b5cd', u'#4c72b0', u'#55a868', u'#c44e52', u'#8172b2',
                 u'#ccb974', u'#64b5cd', u'#4c72b0', u'#55a868', u'#c44e52',
                 u'#8172b2', u'#ccb974', u'#64b5cd', u'#4c72b0', u'#55a868',
                 u'#c44e52', u'#8172b2', u'#ccb974', u'#64b5cd', u'#4c72b0',
...
class AnalysisTrajectories(object):
    def __init__(self, out_dirs, dir_name='run_', analysis_dir='analysis',
                 detect_equilibration=True, burn_in_fraction=0.02,
                 nskip=20, nproc=1, number_models_out=29999, plot_fmt='pdf'):
        '''
        ...
        if plot_fmt not in supported_fmts:
            raise KeyError("plot_fmt not found in supported matplotlib "
                           "file types:", supported_fmts)
        ...
        if self.burn_in_frac > 0:
            print('Considering %1.2f fraction of frames from the beginning '
        ...
        # Other handles
        self.restraint_handles = ['MonteCarlo_Nframe', 'rmf_frame_index',
                                  'Total_Score']
    ...
    def set_analyze_XLs_restraint(self,
        ...
        self.restraint_names[
            'CrossLinkingMassSpectrometryRestraint_PriorPsi_Score'] = 'XLs_psi'
    ...
    def set_analyze_Occams_restraint(self):
        ...
        self.restraint_names[
            'OccamsRestraint_sigma_Score'] = 'Struct_Equiv_sigma'
        ...
        for restraint in self.restraint_handles:
            ...
            if len(RES) == 1:
                ...
                for k, v in RES.items():
                    score_names.append(
                        self.restraint_names[restraint] + '_'
    ...
    def get_distance_fields(self, stat2_dict):
        ...
        for handle in self.distance_handles:
            dist = {stat2_dict[k]: k for k in stat2_dict.keys()
                    if (handle in stat2_dict[k]
    ...
    def get_info_fields(self, stat2_dict):
        ...
        for handle in self.info_handles:
            info = {stat2_dict[k]: k for k in stat2_dict.keys()
                    if (handle in stat2_dict[k]
    ...
    def get_keys(self, stat_file):
        ...
        for line in stat_file_lines:
            ...
            if "STAT2HEADER" in klist:
                ...
                kkeys = [k[0] for k in sorted(stat2_dict.items(),
                                              key=operator.itemgetter(1))]
                klist = [k[1] for k in sorted(stat2_dict.items(),
                                              key=operator.itemgetter(1))]
                invstat2_dict = {}
                for k in kkeys:
    ...
    def read_stat_files(self):
        ...
        out_dirs_dict[self.nproc-1] = \
            list(self.out_dirs[((self.nproc-1)*ND):(len(self.out_dirs))])

        # Setup a list of processes that we want to run
        processes = [mp.Process(target=self.read_traj_info,
                                args=((out_dirs_dict[x], )))
        ...
        for sf in stat_files:
            ...
            for line in sf_lines:
                ...
                if line_number > 1:
                    ...
                    s0 = [float(d[field])
                          for field in score_fields] + [traj, frmf]
                    S_scores.append(s0)
                    if len(dist_fields) > 0:
                        d0 = [s0[0]] + [
                            float(d[field]) for field in dist_fields]
                        S_dist.append(d0)
                    if len(info_fields) > 0:
                        p0 = [s0[0]] + [
                            float(d[field]) for field in info_fields]
                        P_info.append(p0)
    ...
    def read_traj_info(self, out_dirs_sel):
        ...
        for out in out_dirs_sel:
            ...
            if self.Connectivity_restraint \
                ...
                sel_entries += [v for v in S_tot_scores.columns.values
            ...
            self.plot_scores_restraints(
                S_tot_scores[['MC_frame']+sel_entries], ts_eq, burn_in,
                file_out)

            if self.pEMAP_restraint_new:
                file_out_pemap = 'plot_pEMAP_%s.%s' \
                    % (traj_number, self.plot_fmt)
            ...
            if self.XLs_restraint:
                S_info = self.analyze_trajectory_XLs(S_dist, S_info,
                                                     atomic_XLs=False,
            ...
            if out in self.dir_halfA:
                S_tot_scores = S_tot_scores.assign(
                    half=pd.Series(['A']*len(S_tot_scores),
                                   index=S_tot_scores.index).values)
            elif out in self.dir_halfB:
                S_tot_scores = S_tot_scores.assign(
                    half=pd.Series(['B']*len(S_tot_scores),
                                   index=S_tot_scores.index).values)
            else:
                S_tot_scores = S_tot_scores.assign(
                    half=pd.Series([0]*len(S_tot_scores),
                                   index=S_tot_scores.index).values)
    ...
    def plot_scores_restraints(self, selected_scores, ts_eq, burn_in,
        ...
        for i, c in enumerate(selected_scores.columns.values[1:]):
            axes[i].plot(selected_scores['MC_frame'].loc[burn_in::10],
        ...
        for i, c in enumerate(selected_scores.columns.values[1:]):
            axes[i+n_res].hist(selected_scores[c].loc[ts_eq[i]::10],
                               n_bins, histtype='step', fill=False,
    ...
    def write_models_info(self):
        ...
        for k, T in self.S_all.items():
            ...
            T.to_csv(os.path.join(os.path.join(
                self.analysis_dir, 'scores_info_'+str(kk)+'.csv')))

        for k in self.S_dist_all.keys():
            T = self.S_dist_all[k]
            kk = k.split(self.dir_name)[-1].split('/')[0]
            T.to_csv(os.path.join(os.path.join(
                self.analysis_dir, 'XLs_dist_info_'+str(kk)+'.csv')))

        for k in self.S_info_all.keys():
            T = self.S_info_all[k]
            kk = k.split(self.dir_name)[-1].split('/')[0]
            T.to_csv(os.path.join(os.path.join(
                self.analysis_dir, 'other_info_'+str(kk)+'.csv')))
    ...
    def read_models_info(self, XLs_cutoffs=None):
        ...
        info_files = glob.glob(
            os.path.join(self.analysis_dir, 'scores_info_*.csv'))
        for f in info_files:
        ...
        # XLs files
        xls_files = glob.glob(
            os.path.join(self.analysis_dir, 'XLs_dist_info_*.csv'))
        if len(xls_files) > 0:
            self.XLs_restraint = True
            self.ambiguous_XLs_restraint = False
            for f in xls_files:
    ...
    def hdbscan_clustering(self, selected_scores, min_cluster_size=150,
                           min_samples=5, skip=1):
        '''
        ...
        # Add clusters labels
        S_comb_sel = S_comb_sel.assign(cluster=pd.Series(
            hdbsc.labels_, index=S_comb_sel.index).values)
        S_comb_all = S_comb_all.assign(cluster=pd.Series(
            hdbsc.labels_, index=S_comb_all.index).values)

        # Add cluster labels also to XLs info if available
        if self.XLs_restraint is True:

            all_dist_dfs = [self.S_dist_all[dd]
                            for dd in np.sort(self.S_dist_all.keys())]
            S_comb_dist_clustering = pd.concat(
                all_dist_dfs, sort=False).iloc[::skip]

            S_comb_dist_clustering = S_comb_dist_clustering.assign(
                cluster=pd.Series(hdbsc.labels_,
                                  index=S_comb_dist_clustering.index).values)
            S_comb_dist_clustering.to_csv(os.path.join(
                self.analysis_dir, 'XLs_clustering_info.csv'), index=False)
        ...
        S_comb_sel = S_comb_sel.assign(
            half=pd.Series(S_comb_all['half'],
                           index=S_comb_sel.index).values)
    ...
    def write_summary_hdbscan_clustering(self, S_comb_all):
        ...
        aggregation = {v: 'mean' for v in S_comb_all.columns.values
        ...
        columns = ['Total_Score'] + \
                  [x for x in S_clusters.columns.values
                   if x not in ['cluster', 'Total_Score', 'N_models', 'N_A',
                                'N_B']] + \
                  ['N_models', 'N_A', 'N_B']
        S_clusters = S_clusters[columns]
        S_clusters.to_csv(os.path.join(
            self.analysis_dir, 'summary_hdbscan_clustering.dat'), index=True)
        ...
        for cl in clusters:
            ...
            if len(HA['half'].values) >= 0 and len(HB['half'].values) >= 0:
                for traj in counts_traj_A['traj'].values:
                    counts_traj_A.loc[
                        counts_traj_A['traj'] == traj, 'counts_A'] = \
                            len(HA[HA['traj'] == traj])
                for traj in counts_traj_B['traj'].values:
                    counts_traj_B.loc[
                        counts_traj_B['traj'] == traj, 'counts_B'] = \
                            len(HB[HB['traj'] == traj])
                n_max = np.max(list(counts_traj_A['counts_A'].values)
                ...
                axes[0].bar(np.arange(len(runs_A)), counts_traj_A['counts_A'],
                            color='gold', label=runs_A)
                ...
                axes[1].bar(np.arange(len(runs_B)), counts_traj_B['counts_B'],
                            color='orangered', label=runs_B)
    ...
    def write_hdbscan_clustering(self, S_comb):
        ...
        try:
            os.remove(os.path.join(
                self.analysis_dir, 'selected_models_A_cluster*'))
            os.remove(os.path.join(
                self.analysis_dir, 'selected_models_B_cluster*'))
        except:  # noqa: E722
            pass
        ...
        for cl in clusters:
            ...
            if len(HA['half'].values) >= 10 and len(HB['half'].values) >= 10:
                ...
                if (int(n) > self.number_models_out
                    ...
                    HH_sel_A.to_csv(os.path.join(
                        self.analysis_dir,
                        'selected_models_A_cluster' + str(cl)
                        + '_detailed_random.csv'))
                    HH_sel_B.to_csv(os.path.join(
                        self.analysis_dir,
                        'selected_models_B_cluster' + str(cl)
                        + '_detailed_random.csv'))

        if clus_sel == 0:
            print('WARNING: No models were selected because the '
    ...
    def plot_hdbscan_clustering(self, S_comb_sel, selected_scores):
        ...
        for s1, s2 in itertools.product(selected_scores, repeat=2):
            ax = pl.subplot(gs[i])
            if s1 == s2:
                ax.hist(S_comb_sel[s1], 20, histtype='step', color='b',
                        alpha=0.5)
            else:
                ax.scatter(S_comb_sel[s2], S_comb_sel[s1],
                           c=np.array(cluster_colors), s=3.0, alpha=0.3)
    ...
    def do_extract_models(self, gsms_info, filename, gsms_dir):
        ...
        # Setup a list of processes that we want to run
        processes = [mp.Process(target=self.extract_models,
        ...
        np.savetxt(
            os.path.join(gsms_dir, filename+'.txt'), np.array(self.scores))

    def write_GSMs_info(self, gsms_info, filename):
        gsms_info.to_csv(
            os.path.join(self.analysis_dir, filename), index=False)
    ...
    def do_extract_models_single_rmf(
            self, gsms_info, out_rmf_name, traj_dir, analysis_dir,
            scores_prefix="scores", clean_rmfs=True, sel_state=0):
        '''
    ...
    def extract_models_to_single_rmf(self, inf, output_rmf, top_dir,
                                     scores_file, sel_state=0):
    ...
    def plot_scores_distributions(self, HA, HB, cl):
        ...
        # Plot distributions
        ax = pl.subplot(gs[0])
        ax.hist(scores_A, n_bins, histtype='step', stacked=True,
                fill=False, color='orangered')
        ax.hist(scores_B, n_bins, histtype='step', stacked=True,
        ...
        M = np.arange(int(min(len(scores_A), len(scores_B))/20.),
                      min(len(scores_A), len(scores_B)),
                      int(nn/20.))
        if len(M) <= 10.0:
            M = np.arange(int(min(len(scores_A), len(scores_B))/10.),
                          min(len(scores_A), len(scores_B)),
                          int(min(len(scores_A), len(scores_B))/10.))
        ...
        ax = pl.subplot(gs[1])
        ax.errorbar(RH1[:, 0], RH1[:, 1], yerr=RH1[:, 2], c='orangered',
                    fmt='o')
    ...
    def get_psi_stats(self):
        ...
        XLs_nuis = [k for k in self.S_info_all[key_0].columns.values
                    if 'CrossLinkingMassSpectrometryRestraint_Psi_' in k]
        occams_nuis = [k for k in self.S_info_all[key_0].columns.values
        ...
        for k, v in self.S_info_all.items():
            ...
            DF_stat_nuis = DF_stat_nuis.append(
                pd.Series([k]+sel_nuis_mean+sel_nuis_std,
    ...
    def analyze_trajectory_XLs(self, S_dist, S_info, atomic_XLs,
                               traj_number, ts_max):
        sel_XLs_nuis = [v for v in S_info.columns.values
    ...
    def get_XLs_satisfaction(self, S_dist, atomic_XLs, type_XLs=None,
        ...
        if self.ambiguous_XLs_restraint is True:
            ambiguous_XLs_dict = \
                self.check_XLs_ambiguity(S_dist.columns.values)
    ...
    def summarize_XLs_info(self, Multiple_XLs_restraints=False,
                           ambiguous_XLs_restraint=False):
        # Remove files from previous runs
        try:
            os.remove(os.path.join(self.analysis_dir, 'plot_XLs_distances_*'))
            os.remove(os.path.join(self.analysis_dir,
                                   'XLs_satisfaction_cluster_*'))
            os.remove(os.path.join(self.analysis_dir,
                                   'XLs_distances_cluster_*'))
        except:  # noqa: E722
            pass
        ...
        if self.Multiple_XLs_restraints:
            for type_XLs in self.XLs_cutoffs.keys():
                ...
                for cl in unique_clusters:
                    ...
                    self.boxplot_XLs_distances(cluster=cl, type_XLs=type_XLs,
                                               cutoff=cutoff)
    ...
    def get_XLs_details(self, cluster=0, type_XLs=None):
        ...
        if cluster != 'All':
            dXLs_cluster = self.S_comb_dist_clustering.loc[
                self.S_comb_dist_clustering['cluster'] == cluster,
                dist_columns]
        else:
    ...
    def summarize_sampling_info(self):
        ...
        self.Sampling['Number_of_replicas'] = \
            list(set(self.Sampling['Number_of_replicas']))[0]
        self.Sampling['N_total'] = np.sum(self.Sampling['N_total'])
        self.Sampling['N_equilibrated'] = \
            np.sum(self.Sampling['N_equilibrated'])
        ...
        DS.to_csv(os.path.join(self.analysis_dir,
    ...
    def plot_XLs_satisfaction(self, S_info, ts_max, file_out):
        ...
        XLs_nuis = [k for k in S_info.columns.values
                    if 'CrossLinkingMassSpectrometryRestraint_Psi_' in k]
        ...
        for i, c in enumerate(XLs_nuis):
            label = c.split('CrossLinkingMassSpectrometryRestraint_')[-1]
            axes[2].hist(S_info[c].loc[ts_max:], n_bins, histtype='step',
    ...
    def boxplot_XLs_distances(self, cluster=0, type_XLs=None, cutoff=30.0):
        if type_XLs:
            file_out = 'plot_XLs_distance_distributions_cl' + str(cluster) \
                + '_' + str(type_XLs) + '.' + self.plot_fmt
        else:
            file_out = 'plot_XLs_distance_distributions_cl' + str(cluster) \
                + '.' + self.plot_fmt
        ...
        if type_XLs:
            dist_columns = [
                x for x in self.S_comb_dist_clustering.columns.values
                if ('Distance_' in x) and (type_XLs in x)]
        else:
            dist_columns = [
                x for x in self.S_comb_dist_clustering.columns.values
        ...
        # For plot layout
        S_sorted = np.array(
            dXLs_unique)[:, XLs_ids['entry'].values.astype('int')]
        ...
        if type_XLs:
            file_out_hist = 'plot_XLs_distance_histogram_cl' + str(cluster) \
                + '_' + str(type_XLs) + '.' + self.plot_fmt
        else:
            file_out_hist = 'plot_XLs_distance_histogram_cl' + str(cluster) \
                + '.' + self.plot_fmt
    ...
    def plot_pEMAP_satisfaction(self, S_info, file_out):
        ...
        pemap_satif = [v for v in S_info.columns.values
                       if 'pEMapRestraint_satisfaction' in v][0]

        ax.plot(S_info['MC_frame'].iloc[::10], S_info[pemap_satif].iloc[::10],
    ...
    def plot_Occams_satisfaction(self, Occams_info, file_out):
        ...
        # Occams satisfaction
        occams_satif = [k for k in Occams_info.columns.values
                        if 'OccamsRestraint_satisfied' in k]

        # Occams Psi nuisances
        occams_psi_nuis = [k for k in Occams_info.columns.values
                           if ('OccamsRestraint' in k) and ('psi' in k)]

        # Occams Sigma nuisances
        occams_sigma_nuis = [k for k in Occams_info.columns.values
                             if ('OccamsRestraint' in k) and ('sigma' in k)]

        for i, k in enumerate(occams_satif):
            label = k
            ax[0].plot(Occams_info['MC_frame'].loc[::20],
                       Occams_info[k].loc[::20], alpha=0.8, label=label)
        ...
        for i, k in enumerate(occams_psi_nuis):
            label = k
            ax[1].plot(Occams_info['MC_frame'].loc[::20],
                       Occams_info[k].loc[::20], alpha=0.8, label=label)
        ...
        for i, k in enumerate(occams_sigma_nuis):
            label = k
            ax[2].plot(Occams_info['MC_frame'].loc[::10],
                       Occams_info[k].loc[::10], color=c[i], alpha=0.8,
                       label=label)
    ...
    def get_str_match(self, strs):
        if len(strs) > 1:
            intersect = \
                set(self.substrings(strs[0])) & set(self.substrings(strs[1]))
