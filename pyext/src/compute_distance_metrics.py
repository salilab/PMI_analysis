...
class get_distance_metrics(object):
    ...
    def compute_DRMSD_all_versus_centroid(self):
        rmf3 = os.path.join(self.clustering_dir,'cluster.'+str(self.cluster),'cluster_center_model.rmf3')
    ...
    def write_RMSD_output(self):
        ...
        file_dev_matrix = os.path.join(
            out_dir,'RMSD_dev_pairs_all_versus_centroid.csv')
        ...
        for pair, values in  self.RMSD_pairs.items():
            ...
            M.loc[pair[0], pair[1]] = np.mean(values)
            M.loc[pair[1], pair[0]] = M.loc[pair[0], pair[1]]
            file_out = os.path.join(out_dir,file_values+str(pair[0])+'_'+str(pair[1])+'.csv')
